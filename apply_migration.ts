...
const sql0001 = `
ALTER TABLE \`availability_exceptions\` ADD \`break_start\` text;--> statement-breakpoint
ALTER TABLE \`availability_exceptions\` ADD \`break_end\` text;
`;
...
async function main() {
    ...
    // Combine all SQL
    const allSql = sql0000 + "--> statement-breakpoint" + sql0001;

    const statements = allSql.split('--> statement-breakpoint')
        .map(s => s.trim())
        .filter(s => s.length > 0);
    ...
    for (const stmt of statements) {
        ...
        try {
            ...
            // Optionally ignore "table already exists" if re-running
            if (e.message.includes("already exists")) {
                console.log("  (Ignored: already exists)");
            } else {
                // throw e; // Don't throw, try to continue or check
            }
        }
    }
